...
// Futures features avec badge "Bientôt"
const COMING_SOON = [
  { label: 'Marketplace',   icon: '🏪', feature: 'carbon_marketplace' },
  { label: 'Assistant IA',  icon: '🤖', feature: 'ai_assistant' },
  { label: 'Import CSV',    icon: '📥', feature: 'bulk_import' },
];
...
function SidebarContent({ user, logout }: { user: any; logout: () => void }) {
  ...
  const visibleNav = STATIC_NAV.filter(item =>
    item.feature === null || flags[item.feature] === true
  );

  const visibleComingSoon = COMING_SOON.filter(item => flags[item.feature] === true);
  ...
  return (
    <>
      ...
      <nav className="flex-1 py-3" style={{ overflowY: 'auto' }}>
        ...
        {/* Features activées depuis admin */}
        {visibleComingSoon.length > 0 && (
          <div style={{ margin: '8px 8px 4px', paddingTop: 8, borderTop: '1px solid #1E2D3D' }}>
            <div style={{ fontSize: 9, color: '#2A3F55', fontFamily: 'JetBrains Mono, monospace', padding: '0 8px 6px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Nouvelles features</div>
            {visibleComingSoon.map(item => (
              <div key={item.feature} className="nav-item" style={{ cursor: 'default', opacity: 0.7 }}>
                <span style={{ fontSize: 13 }}>{item.icon}</span>
                {item.label}
                <span style={{ marginLeft: 'auto', fontSize: 9, background: 'rgba(0,255,148,0.15)', color: '#00CC77', borderRadius: 3, padding: '1px 4px', fontFamily: 'JetBrains Mono, monospace' }}>NEW</span>
              </div>
            ))}
          </div>
        )}
