...
// ─── 9 standards rapport (tous actifs) ───────────────────────────────────────
const AUDIT_STANDARDS = [
  ...
  { id:'BILAN_CARBONE', nameEn:'Bilan Carbone®',     nameFr:'Bilan Carbone®',         icon:'🇫🇷', color:'#059669', edition:'ADEME v8',               badgeEn:'ADEME France/Africa',  badgeFr:'ADEME France/Afrique' },
  ...
  { id:'VCMI_CCP',      nameEn:'VCMI Claims Code',   nameFr:'VCMI Réclamations',      icon:'🏆', color:'#0E7490', edition:'VCMI 2024',              badgeEn:'Voluntary Markets',    badgeFr:'Marchés Volontaires' },
];
...
export default function GHGAuditPage() {
  ...
  const loadAudit = useCallback(async (id) => {
    ...
    try {
      ...
      setCurrentAudit(a); setView('audit'); setShowReportPanel(false);
  ...
  const createAudit = async () => {
    ...
    try {
      ...
      await load(); await loadAudit(audit.id);
  ...
  const addEntryFn = async () => {
    ...
    try {
      ...
      setCurrentAudit(updated); setOffsetPlan(plan);
  ...
  const executeDeleteEntry = async () => {
    ...
    const eid = confirmDeleteEntry; setConfirmDeleteEntry(null);
    try {
  ...
  const confirmDeleteAudit = async () => {
    ...
    try {
      ...
      setDeleteConfirm(null); setCurrentAudit(null); setView('dashboard'); await load();
    } catch(e) { showToast(e.message,'error'); }
  };
  ...
  const downloadAuditReport = async (auditId, auditName, reportingYear, stdId, pdfLang) => {
    const key = auditId+'-'+stdId+'-'+pdfLang;
    setGeneratingKey(key);
    showToast(L('Generating','Génération')+' '+stdId+' ('+pdfLang.toUpperCase()+')...','info');
    try {
      const token = typeof window!=='undefined'?localStorage.getItem('accessToken'):'';
      const url = (process.env.NEXT_PUBLIC_API_URL||'')+'/ghg/audits/'+auditId+'/report?lang='+pdfLang+'&standard='+stdId;
      ...
      a.href=objUrl;
      a.download='PANGEA-GHG-'+stdId+'-'+(auditName||'Audit').replace(/[^a-zA-Z0-9]/g,'-').slice(0,20)+'-'+(reportingYear||2024)+'-'+pdfLang.toUpperCase()+'.pdf';
  ...
  const factorsByScope = useMemo(()=>{
    const g = {};
    factors.forEach(f=>{ if(!g[f.scope])g[f.scope]={}; if(!g[f.scope][f.cat])g[f.scope][f.cat]=[]; g[f.scope][f.cat].push(f); });
    return g;
  },[factors]);
  ...
  const scopeChartData = useMemo(()=>{
    ...
    return [1,2,3].map(s=>{
      ...
      return { name:'S'+s, value:+val.toFixed(2), pct:+((val/total)*100).toFixed(1), color:SC[s] };
    }).filter(d=>d.value>0);
  },[currentAudit]);
  ...
  const categoryChartData = useMemo(()=>{
    ...
    const cats = {};
    currentAudit.entries.forEach(e=>{ const cat=e.category||'Other'; cats[cat]=(cats[cat]||0)+e.co2e; });
  ...
  const TT = ({ active, payload, label }) => {
    ...
    return (
      <div style={{ background:C.card, border:'1px solid '+C.border, borderRadius:8, padding:'10px 14px', fontSize:11 }}>
        ...
        {payload.map((p,i)=>(<div key={i} style={{ color:p.color||C.text }}>{p.name}: {p.value} tCO₂e</div>))}
      </div>
    );
  };

  // ─── PANGEA Modal component ───────────────────────────────────────────────
  const PangeaModal = ({ open, onClose, title, titleFr, eyebrow, icon, accentColor, children }) => {
    if (!open) return null;
    const col = accentColor || C.red;
    return (
      <div onClick={e=>{ if(e.target===e.currentTarget)onClose(); }}
        style={{ position:'fixed',inset:0,background:'rgba(8,11,15,0.9)',backdropFilter:'blur(12px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:10002,padding:16 }}>
        <div style={{ background:C.card,border:'1px solid '+col+'35',borderRadius:18,padding:28,maxWidth:480,width:'100%',boxShadow:'0 32px 80px rgba(0,0,0,0.8)',position:'relative',overflow:'hidden' }}>
          <div style={{ position:'absolute',top:0,left:0,right:0,height:3,background:'linear-gradient(90deg,'+col+' 0%,'+col+'30 70%,transparent 100%)' }}/>
          <div style={{ display:'flex',gap:14,alignItems:'center',marginBottom:16 }}>
            <div style={{ width:52,height:52,borderRadius:13,background:col+'12',border:'1px solid '+col+'30',display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,flexShrink:0 }}>{icon}</div>
            <div>
              <div style={{ fontSize:9,color:col,fontFamily:'JetBrains Mono, monospace',letterSpacing:'0.13em',marginBottom:4,textTransform:'uppercase' }}>
                GHG AUDIT · {eyebrow}
              </div>
              <h2 style={{ fontFamily:'Syne, sans-serif',fontSize:17,fontWeight:800,color:col,margin:0 }}>
                {lang==='fr'?titleFr:title}
              </h2>
            </div>
          </div>
          <div style={{ height:1,background:'linear-gradient(90deg,'+col+'30 0%,transparent 100%)',marginBottom:20 }}/>
          {children}
        </div>
      </div>
    );
    ...
    <div style={{ padding:20, maxWidth:1500, margin:'0 auto' }}>
      ...
      {toast && (
        <div style={{ position:'fixed',top:24,right:24,zIndex:99999,maxWidth:440,minWidth:280 }}>
          <div style={{
            background: toast.type==='error'?'rgba(248,113,113,0.06)':toast.type==='info'?'rgba(56,189,248,0.06)':'rgba(0,255,148,0.06)',
            border:'1px solid '+(toast.type==='error'?'rgba(248,113,113,0.35)':toast.type==='info'?'rgba(56,189,248,0.3)':'rgba(0,255,148,0.3)'),
            borderRadius:14, padding:'14px 18px', display:'flex', alignItems:'center', gap:12,
            backdropFilter:'blur(20px)', boxShadow:'0 8px 40px rgba(0,0,0,0.6)', position:'relative', overflow:'hidden'
          }}>
            <div style={{ position:'absolute',left:0,top:0,bottom:0,width:3,borderRadius:'14px 0 0 14px',background:toast.type==='error'?C.red:toast.type==='info'?C.blue:C.green }}/>
            <div style={{ width:24,height:24,borderRadius:'50%',background:(toast.type==='error'?C.red:toast.type==='info'?C.blue:C.green)+'18',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,color:toast.type==='error'?C.red:toast.type==='info'?C.blue:C.green,fontWeight:800,marginLeft:6 }}>
              {toast.type==='error'?'✗':toast.type==='info'?'◌':'✓'}
            </div>
            <span style={{ fontSize:13,color:C.text,flex:1,lineHeight:1.4 }}>{toast.msg}</span>
            <button onClick={()=>setToast(null)} style={{ background:'transparent',border:'none',color:C.muted,cursor:'pointer',fontSize:16,padding:'0 4px' }}>×</button>
          </div>
        </div>
      )}

      {/* ── MODAL: Supprimer Audit ─────────────────────────────────────────── */}
      <PangeaModal open={!!deleteConfirm} onClose={()=>setDeleteConfirm(null)}
        title="Delete this audit?" titleFr="Supprimer cet audit ?"
        eyebrow={L('DELETE','SUPPRESSION')} icon="🗑" accentColor={C.red}>
        <div style={{ background:'rgba(248,113,113,0.05)',border:'1px solid rgba(248,113,113,0.15)',borderRadius:10,padding:'14px 16px',marginBottom:20 }}>
          ...
          <p style={{ fontSize:12,color:C.text2,margin:0,lineHeight:1.7 }}>
            {L('All emission entries and AI analyses will be permanently deleted. This action is irreversible.','Toutes les entrées et analyses IA seront définitivement supprimées. Cette action est irréversible.')}
          </p>
        </div>
      ...
      <PangeaModal open={!!confirmDeleteEntry} onClose={()=>setConfirmDeleteEntry(null)}
        title="Delete this entry?" titleFr="Supprimer cette entrée ?"
        eyebrow={L('DELETE ENTRY','SUPPRESSION ENTRÉE')} icon="📋" accentColor={C.purple}>
        <p style={{ fontSize:12,color:C.text2,marginBottom:20,lineHeight:1.7 }}>
          {L('This emission entry will be permanently removed and scope totals recalculated.','Cette entrée sera supprimée définitivement et les totaux de scope recalculés.')}
        </p>
        <div style={{ display:'flex',gap:10 }}>
      ...
      <div style={{ marginBottom:24, display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          <div style={{ fontSize:9,color:C.blue,fontFamily:'JetBrains Mono, monospace',letterSpacing:'0.14em',marginBottom:6 }}>
            GHG PROTOCOL · ISO 14064 · BILAN CARBONE · CSRD · TCFD · SBTi · CDP · SEC · VCMI
          </div>
          ...
          <p style={{ fontSize:13,color:C.muted,margin:0 }}>
            {L('Measure · Reduce · Offset — Scope 1, 2 & 3 — 9 standards × EN/FR — 18 PDF reports per audit','Mesurer · Réduire · Compenser — Scope 1, 2 & 3 — 9 standards × EN/FR — 18 rapports PDF par audit')}
          </p>
        </div>
        <div style={{ display:'flex',gap:8,flexShrink:0 }}>
          {view !== 'dashboard' && (
            <>
              <button onClick={()=>{ setView('dashboard'); setCurrentAudit(null); setOffsetPlan(null); setActiveTab('entries'); setShowReportPanel(false); }}
      ...
      {view==='dashboard' && (
        <>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20 }}>
            {[
              ...
              <div key={k.l} style={{ background:C.card,border:'1px solid '+k.c+'22',borderRadius:12,padding:'14px 18px',position:'relative',overflow:'hidden' }}>
                ...
                <div style={{ position:'absolute',top:14,right:14,fontSize:22,opacity:0.2 }}>{k.icon}</div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 360px',gap:16 }}>
            ...
            <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:14,overflow:'hidden' }}>
              ...
              {audits.length===0 ? (
                <div style={{ padding:56,textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:12,color:C.muted,marginBottom:20,lineHeight:1.7,maxWidth:360,margin:'0 auto 20px' }}>
                    {L('Start your first GHG audit. Measure Scope 1, 2 & 3 with IPCC AR6 + IEA 2024 factors. Generate 18 certified PDFs.','Démarrez votre premier audit GHG. Mesurez Scope 1, 2 & 3 avec les facteurs IPCC AR6 + AIE 2024. Générez 18 PDFs certifiés.')}
                  </div>
                  <button onClick={()=>setView('new')} style={{ background:C.green,color:C.bg,border:'none',borderRadius:9,padding:'12px 28px',fontWeight:800,cursor:'pointer',fontFamily:'Syne, sans-serif' }}>
                    ...
                    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10 }}>
                      <div>
                        <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:4 }}>
                          <span style={{ fontSize:14 }}>{fw?.flag||'📊'}</span>
                      ...
                      <div style={{ textAlign:'right',flexShrink:0 }}>
                        <span style={{ fontSize:8,color:sc,background:sc+'15',border:'1px solid '+sc+'30',borderRadius:4,padding:'2px 8px',fontFamily:'JetBrains Mono, monospace',display:'block',marginBottom:6 }}>{a.status}</span>
                        ...
                        return (
                          <div key={s}>
                            <div style={{ display:'flex',justifyContent:'space-between',marginBottom:3 }}>
                              <span style={{ fontSize:9,color:SC[s],fontFamily:'JetBrains Mono, monospace' }}>S{s}: {fmtT(val)} t</span>
                              <span style={{ fontSize:9,color:C.muted }}>{pct}%</span>
                            </div>
                            <div style={{ height:3,background:C.border,borderRadius:2 }}>
                              <div style={{ width:pct+'%',height:'100%',background:SC[s],borderRadius:2 }}/>
                            </div>
                          </div>
                        );
                      })}
            ...
            {/* Right: 9 standards + bridge */}
            <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
              {/* 9 standards PDF grid */}
              <div style={{ background:C.card,border:'1px solid rgba(0,255,148,0.2)',borderRadius:14,padding:16,position:'relative',overflow:'hidden' }}>
                ...
                <div style={{ fontSize:9,color:C.green,fontFamily:'JetBrains Mono, monospace',marginBottom:12,letterSpacing:'0.1em' }}>
                  📥 9 {L('STANDARDS × 2 LANGUAGES = 18 PDFs / AUDIT','STANDARDS × 2 LANGUES = 18 PDFs / AUDIT')}
                </div>
                <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6 }}>
                  {AUDIT_STANDARDS.map(std=>(
                    <div key={std.id} style={{ background:C.card2,border:'1px solid '+std.color+'20',borderRadius:8,padding:'8px 10px' }}>
                      <div style={{ fontSize:14,marginBottom:3 }}>{std.icon}</div>
                      <div style={{ fontSize:9,fontWeight:700,color:std.color,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>
                        {lang==='fr'?std.nameFr:std.nameEn}
                      </div>
                      <div style={{ fontSize:7,color:C.muted,fontFamily:'JetBrains Mono, monospace' }}>{std.edition}</div>
                    </div>
                  ))}
                </div>
                <div style={{ marginTop:10,fontSize:10,color:C.muted,textAlign:'center' }}>
                  {L('Open any audit → click','Ouvrir un audit → cliquer')} <span style={{ color:C.blue }}>📥 {L('Reports × 9','Rapports × 9')}</span>
                </div>
              </div>
              ...
              {dashboardScopeData.length > 0 && (
                <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:12,padding:14 }}>
                  ...
                  <ResponsiveContainer width="100%" height={120}>
                    <PieChart>
                      <Pie data={dashboardScopeData} dataKey="value" cx="50%" cy="50%" outerRadius={45} innerRadius={22}>
                  ...
                  <div style={{ display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap' }}>
                    {dashboardScopeData.map(d=>(
                      <span key={d.name} style={{ fontSize:9,color:d.color,fontFamily:'JetBrains Mono, monospace' }}>■ {d.name}: {d.value}t</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div style={{ background:'rgba(56,189,248,0.05)',border:'1px solid rgba(56,189,248,0.15)',borderRadius:10,padding:'12px 14px' }}>
                ...
                <div style={{ fontSize:10,color:C.text2,lineHeight:1.6 }}>
                  {L('CSRD mandates Scope 1+2+3 GHG reporting for EU companies >250 employees. PANGEA audits are aligned with EFRAG taxonomy.','La CSRD impose le reporting GHG Scope 1+2+3 pour les entreprises UE >250 salariés. Les audits PANGEA sont alignés EFRAG.')}
                </div>
              </div>

              {/* Offset bridge */}
              <div style={{ background:'rgba(0,255,148,0.05)',border:'1px solid rgba(0,255,148,0.15)',borderRadius:10,padding:'12px 14px' }}>
                <div style={{ fontSize:9,color:C.green,fontFamily:'JetBrains Mono, monospace',marginBottom:6 }}>🌿 AUDIT → OFFSET BRIDGE</div>
                <div style={{ fontSize:10,color:C.text2,lineHeight:1.6,marginBottom:10 }}>
                  {L('After your audit, PANGEA calculates African carbon credits needed to offset your footprint.','Après votre audit, PANGEA calcule les crédits carbone africains pour compenser votre empreinte.')}
                </div>
                <a href="/dashboard/marketplace" style={{ display:'block',background:'rgba(0,255,148,0.08)',border:'1px solid rgba(0,255,148,0.2)',borderRadius:7,padding:'8px 12px',color:C.green,textDecoration:'none',fontSize:11,fontWeight:700,textAlign:'center' }}>
        ...
        <div style={{ maxWidth:640,margin:'0 auto' }}>
          <div style={{ background:C.card,border:'1px solid rgba(0,255,148,0.2)',borderRadius:16,padding:28,position:'relative',overflow:'hidden' }}>
            ...
            <div style={{ marginBottom:16 }}>
              <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:6 }}>{L('AUDIT NAME *','NOM AUDIT *')}</div>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16 }}>
              <div>
                <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:6 }}>{L('REPORTING YEAR','ANNÉE')}</div>
              ...
              <div>
                <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:6 }}>NET ZERO TARGET</div>
            ...
            <div style={{ marginBottom:20 }}>
              <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:10 }}>
                {L('FRAMEWORK — 9 PDF REPORT STANDARDS AVAILABLE','RÉFÉRENTIEL — 9 STANDARDS DE RAPPORT PDF DISPONIBLES')}
              </div>
                ...
                {FRAMEWORKS.map(fw=>{
                  ...
                  return (
                    <button key={fw.id} onClick={()=>setNewAuditForm(f=>({...f,framework:fw.id}))}
                      style={{ padding:'12px 16px',borderRadius:10,border:'1px solid '+(sel?fw.color+'50':C.border),background:sel?fw.color+'08':'transparent',cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:12 }}>
                      ...
                      {sel && <span style={{ color:fw.color,fontWeight:800 }}>✓</span>}
                    </button>
                  );
                })}
              </div>
              <div style={{ marginTop:10,padding:'10px 12px',background:'rgba(56,189,248,0.05)',border:'1px solid rgba(56,189,248,0.15)',borderRadius:8,fontSize:10,color:C.text2 }}>
                💡 {L('Regardless of the framework chosen, you can generate reports for all 9 standards (GHG Protocol, ISO 14064, CSRD, TCFD, SBTi, CDP, SEC, VCMI...) in EN + FR.','Quel que soit le référentiel choisi, vous pouvez générer des rapports pour les 9 standards (GHG Protocol, ISO 14064, CSRD, TCFD, SBTi, CDP, SEC, VCMI...) en EN + FR.')}
              </div>
            </div>
            <div style={{ display:'flex',gap:10 }}>
              <button onClick={()=>setView('dashboard')} style={{ flex:1,background:'transparent',border:'1px solid '+C.border,borderRadius:9,color:C.muted,padding:13,cursor:'pointer' }}>{L('Cancel','Annuler')}</button>
      ...
      {view==='audit' && currentAudit && (
        <>
          {/* Audit header */}
          <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:14,padding:'16px 22px',marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center',gap:12 }}>
            <div>
              <div style={{ display:'flex',gap:8,alignItems:'center',marginBottom:6 }}>
                <span style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',background:C.card2,border:'1px solid '+C.border,borderRadius:4,padding:'2px 8px' }}>{currentAudit.framework}</span>
                <span style={{ fontSize:9,color:STATUS_C[currentAudit.status]||C.muted,background:(STATUS_C[currentAudit.status]||C.muted)+'15',border:'1px solid '+(STATUS_C[currentAudit.status]||C.muted)+'30',borderRadius:4,padding:'2px 8px',fontFamily:'JetBrains Mono, monospace' }}>{currentAudit.status}</span>
            ...
            <div style={{ display:'flex',gap:8,flexShrink:0,flexWrap:'wrap',justifyContent:'flex-end' }}>
              <button onClick={()=>setShowReportPanel(!showReportPanel)}
                style={{ background:showReportPanel?'rgba(0,255,148,0.12)':'rgba(56,189,248,0.08)',border:'1px solid '+(showReportPanel?'rgba(0,255,148,0.35)':'rgba(56,189,248,0.25)'),borderRadius:9,color:showReportPanel?C.green:C.blue,padding:'10px 18px',cursor:'pointer',fontSize:12,fontWeight:700 }}>
                📥 {showReportPanel?L('Hide reports','Masquer rapports'):L('Reports × 9 standards','Rapports × 9 standards')}
              </button>
              <button onClick={runAI} disabled={aiLoading||!(currentAudit.grandTotal>0)}
                style={{ background:aiLoading?C.card2:'rgba(167,139,250,0.12)',border:'1px solid rgba(167,139,250,0.3)',borderRadius:9,color:aiLoading?C.muted:C.purple,padding:'10px 18px',cursor:aiLoading||!currentAudit.grandTotal?'not-allowed':'pointer',fontSize:12,fontWeight:700 }}>
                {aiLoading?'⟳ '+L('Analyzing...','Analyse...'):L('🤖 AI Analysis','🤖 Analyse IA')}
              </button>
            </div>
          </div>

          {/* ── PANEL 9 STANDARDS × 2 LANGUES ─────────────────────────────── */}
          {showReportPanel && (
            <div style={{ background:C.card,border:'1px solid rgba(0,255,148,0.25)',borderRadius:16,padding:20,marginBottom:16,position:'relative',overflow:'hidden' }}>
              <div style={{ position:'absolute',top:0,left:0,right:0,height:3,background:'linear-gradient(90deg,'+C.green+' 0%,rgba(0,255,148,0.3) 60%,transparent 100%)' }}/>
              <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16 }}>
                <div>
                  <div style={{ fontSize:9,color:C.green,fontFamily:'JetBrains Mono, monospace',letterSpacing:'0.1em',marginBottom:4 }}>
                    📥 {L('GHG AUDIT REPORTS — 9 STANDARDS × 2 LANGUAGES = 18 PDF REPORTS',"RAPPORTS D'AUDIT GHG — 9 STANDARDS × 2 LANGUES = 18 RAPPORTS PDF")}
                  </div>
                  <div style={{ fontSize:12,color:C.text2 }}>
                    {L('Click EN or FR to download a certified PDF for each standard. Each report is 3 pages: cover + emissions detail + offset strategy.','Cliquez EN ou FR pour télécharger un PDF certifié pour chaque standard. Chaque rapport fait 3 pages : couverture + détail émissions + stratégie de compensation.')}
                  </div>
                </div>
                <button onClick={()=>setShowReportPanel(false)} style={{ background:'transparent',border:'none',color:C.muted,cursor:'pointer',fontSize:18,padding:'0 6px',flexShrink:0 }}>×</button>
              </div>
                    ...
                    <div key={std.id} style={{ background:C.card2,border:'1px solid '+std.color+'25',borderRadius:12,padding:'14px 16px' }}>
                      <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:8 }}>
                        ...
                        <div style={{ flex:1,minWidth:0 }}>
                          ...
                          <div style={{ fontSize:8,color:C.muted,fontFamily:'JetBrains Mono, monospace' }}>{std.edition}</div>
                        </div>
                      </div>
                      <div style={{ display:'flex',gap:6 }}>
                        <button onClick={()=>downloadAuditReport(currentAudit.id,currentAudit.name,currentAudit.reportingYear,std.id,'en')} disabled={!!isGenEN}
                          style={{ flex:1,background:isGenEN?C.card:'rgba(56,189,248,0.08)',border:'1px solid rgba(56,189,248,0.25)',borderRadius:7,color:isGenEN?C.muted:C.blue,padding:'8px 0',cursor:isGenEN?'wait':'pointer',fontSize:11,fontWeight:700,transition:'all .1s' }}>
                          {isGenEN?'⟳':'📥'} EN
                        </button>
                        <button onClick={()=>downloadAuditReport(currentAudit.id,currentAudit.name,currentAudit.reportingYear,std.id,'fr')} disabled={!!isGenFR}
                          style={{ flex:1,background:isGenFR?C.card:'rgba(167,139,250,0.08)',border:'1px solid rgba(167,139,250,0.25)',borderRadius:7,color:isGenFR?C.muted:C.purple,padding:'8px 0',cursor:isGenFR?'wait':'pointer',fontSize:11,fontWeight:700,transition:'all .1s' }}>
                          {isGenFR?'⟳':'📥'} FR
                        </button>
                      </div>
                    </div>
              ...
              <div style={{ marginTop:12,fontSize:10,color:C.muted,fontFamily:'JetBrains Mono, monospace',textAlign:'center' }}>
                ⚡ {AUDIT_STANDARDS.length} standards × 2 {L('languages','langues')} = {AUDIT_STANDARDS.length*2} PDF · {L('Certifiable by VVB — IPCC AR6 + IEA 2024 factors','Certifiables par VVB — Facteurs IPCC AR6 + AIE 2024')}
              </div>
            </div>
          )}
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:16 }}>
            {[
              ...
              const pct = s.scope>0&&currentAudit.grandTotal>0?+(s.v/currentAudit.grandTotal*100).toFixed(1):100;
              return (
                <div key={s.l} style={{ background:C.card,border:'1px solid '+s.c+'22',borderRadius:12,padding:'16px 18px',position:'relative',overflow:'hidden' }}>
                  ...
                  <div style={{ position:'absolute',top:12,right:14,fontSize:18,opacity:0.2 }}>{s.icon}</div>
                  <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:5 }}>
                    {s.scope>0?'SCOPE '+s.scope+' · ':'TOTAL · '}{pct}%
                  </div>
                  <div style={{ fontSize:20,fontWeight:800,color:s.c,fontFamily:'Syne, sans-serif' }}>{fmtT(s.v)}</div>
                  <div style={{ fontSize:10,color:C.muted,marginBottom:s.scope>0?6:0 }}>tCO₂e</div>
          ...
          {/* Offset plan */}
          {offsetPlan && (
            <div style={{ background:'rgba(0,255,148,0.04)',border:'1px solid rgba(0,255,148,0.2)',borderRadius:12,padding:'16px 20px',marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center',gap:16,flexWrap:'wrap' }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:9,color:C.green,fontFamily:'JetBrains Mono, monospace',marginBottom:6 }}>🌿 {L('RECOMMENDED OFFSET STRATEGY','STRATÉGIE DE COMPENSATION')}</div>
                <div style={{ fontSize:12,color:C.text2,lineHeight:1.7 }}>{offsetPlan.recommendation}</div>
                <div style={{ display:'flex',gap:8,marginTop:10,flexWrap:'wrap' }}>
                  {(offsetPlan.strategy||[]).map(o=>(
                    <div key={o.standard} style={{ background:C.card2,border:'1px solid '+C.border,borderRadius:8,padding:'8px 12px',textAlign:'center',minWidth:110 }}>
                      <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:3 }}>{o.standard.replace('_',' ')}</div>
                      <div style={{ fontSize:13,fontWeight:800,color:C.text }}>{(o.qty||0).toLocaleString()} t</div>
                      <div style={{ fontSize:10,color:C.green,fontWeight:600 }}>{fmtUSD(o.cost)}</div>
                    </div>
                  ))}
                </div>
              </div>
              <a href="/dashboard/marketplace" style={{ background:C.green,color:C.bg,borderRadius:10,padding:'12px 22px',fontWeight:800,fontSize:13,textDecoration:'none',fontFamily:'Syne, sans-serif',flexShrink:0 }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'310px 1fr',gap:16 }}>
            ...
            <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
              <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:14,padding:18 }}>
                <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:14 }}>+ {L('ADD EMISSION SOURCE','AJOUTER UNE SOURCE')}</div>
                ...
                {selectedFactor && (
                  <div style={{ background:'rgba(56,189,248,0.05)',border:'1px solid rgba(56,189,248,0.15)',borderRadius:9,padding:'12px 14px',marginBottom:12 }}>
                    ...
                    <div style={{ fontSize:10,color:C.muted,marginBottom:4 }}>{L('Factor','Facteur')}: {selectedFactor.factor} tCO₂e/{selectedFactor.unit}</div>
                    <div style={{ fontSize:9,padding:'2px 8px',background:SC[selectedFactor.scope]+'15',color:SC[selectedFactor.scope],border:'1px solid '+SC[selectedFactor.scope]+'30',borderRadius:4,display:'inline-block',fontFamily:'JetBrains Mono, monospace' }}>
                      Scope {selectedFactor.scope}
                    </div>
                    {estimatedCO2>0&&<div style={{ marginTop:8,padding:'7px 10px',background:'rgba(248,113,113,0.08)',borderRadius:7,fontSize:14,fontWeight:800,color:C.red,fontFamily:'JetBrains Mono, monospace' }}>≈ {estimatedCO2.toFixed(3)} tCO₂e</div>}
                  </div>
                )}
                {selectedFactor&&(
                  <div style={{ marginBottom:12 }}>
                    <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:6 }}>{L('QUANTITY','QUANTITÉ')} ({selectedFactor.unit}) *</div>
                    <input type="number" step="any" placeholder={'0 '+selectedFactor.unit} value={addEntry.quantity} onChange={e=>setAddEntry(f=>({...f,quantity:e.target.value}))} style={inp}/>
                  </div>
                )}
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:6 }}>NOTES</div>
                  <input placeholder={L('Optional notes...','Notes optionnelles...')} value={addEntry.notes} onChange={e=>setAddEntry(f=>({...f,notes:e.target.value}))} style={inp}/>
                </div>
              ...
              {scopeChartData.length>0&&(
                <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:12,padding:14 }}>
                  <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:8 }}>{L('SCOPE DISTRIBUTION','RÉPARTITION SCOPE')}</div>
                  <ResponsiveContainer width="100%" height={110}>
                    <PieChart>
                      <Pie data={scopeChartData} dataKey="value" cx="50%" cy="50%" outerRadius={42} innerRadius={22}>
                  ...
                  <div style={{ display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap' }}>
                    {scopeChartData.map(d=><span key={d.name} style={{ fontSize:9,color:d.color,fontFamily:'JetBrains Mono, monospace' }}>■ {d.name}: {d.pct}%</span>)}
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
              {/* Tabs */}
              <div style={{ display:'flex',gap:4,background:C.card,border:'1px solid '+C.border,borderRadius:12,padding:5,width:'fit-content' }}>
                {[
                  ...
                  <button key={id} onClick={()=>setActiveTab(id)}
                    style={{ padding:'9px 18px',border:'none',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:'JetBrains Mono, monospace',borderRadius:8,background:activeTab===id?C.green:'transparent',color:activeTab===id?C.bg:C.muted,transition:'all .15s' }}>
                    {label}
                  </button>
                ))}
              </div>
                ...
                <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:14,overflow:'hidden' }}>
                    ...
                    <div style={{ display:'flex',gap:4,marginLeft:'auto' }}>
                      {[0,1,2,3].map(s=>(
                        <button key={s} onClick={()=>setFilterScope(s===filterScope?0:s)}
                          style={{ padding:'5px 10px',borderRadius:6,border:'1px solid '+(s===filterScope?(s===0?C.muted:SC[s]):C.border),background:s===filterScope?(s===0?C.muted:SC[s])+'15':'transparent',color:s===filterScope?(s===0?C.text2:SC[s]):C.muted,cursor:'pointer',fontSize:11,fontFamily:'JetBrains Mono, monospace' }}>
                          {s===0?L('All','Tout'):'S'+s}
                        </button>
                      ))}
                    </div>
                  ...
                  {filteredEntries.length===0?(
                    <div style={{ padding:'40px 0',textAlign:'center',color:C.muted,fontSize:13 }}>
                      {currentAudit.entries?.length===0?L('No entries — add your first source','Aucune entrée — ajoutez une source'):L('No entries match filter','Aucune entrée pour ce filtre')}
                    </div>
                  ):(
                    <table style={{ width:'100%',borderCollapse:'collapse',fontSize:11 }}>
                      <thead>
                        <tr style={{ background:'rgba(255,255,255,0.02)' }}>
                          {['Scope',L('Source','Source'),L('Qty','Qté'),L('Unit','Unité'),'EF','tCO₂e',''].map(h=>(
                        ...
                        {filteredEntries.map(e=>(
                          <tr key={e.id} style={{ borderBottom:'1px solid '+C.border+'22' }}
                            ...
                            <td style={{ padding:'9px 12px' }}>
                              <span style={{ fontSize:9,color:SC[e.scope],background:SC[e.scope]+'15',border:'1px solid '+SC[e.scope]+'30',borderRadius:3,padding:'1px 6px',fontFamily:'JetBrains Mono, monospace' }}>S{e.scope}</span>
                            </td>
                            ...
                            <td style={{ padding:'9px 12px' }}>
                              <button onClick={()=>setConfirmDeleteEntry(e.id)} style={{ background:'transparent',border:'none',color:C.muted,cursor:'pointer',fontSize:15,padding:'2px 6px' }}>×</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  {categoryChartData.length>0?(
                    <>
                      <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:14,padding:16 }}>
                        <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:10 }}>{L('TOP EMISSION SOURCES (tCO₂e)','TOP SOURCES (tCO₂e)')}</div>
                      ...
                      <div style={{ background:C.card,border:'1px solid '+C.border,borderRadius:14,padding:16 }}>
                        <div style={{ fontSize:9,color:C.muted,fontFamily:'JetBrains Mono, monospace',marginBottom:10 }}>{L('SCOPE BREAKDOWN','RÉPARTITION SCOPE')}</div>
                        <ResponsiveContainer width="100%" height={160}>
                          <BarChart data={[{ name:String(currentAudit.reportingYear), s1:currentAudit.scope1Total||0, s2:currentAudit.scope2Total||0, s3:currentAudit.scope3Total||0 }]}>
                            <XAxis dataKey="name" tick={{ fontSize:10,fill:C.muted }}/>
                            <YAxis tickFormatter={v=>v+'t'} tick={{ fontSize:10,fill:C.muted }} width={50}/>
                            <Tooltip content={<TT/>}/>
                            <Bar dataKey="s1" name="Scope 1" fill={C.red} stackId="a"/>
                  ...
                  {currentAudit.aiAnalysis?(
                    ...
                    <div style={{ textAlign:'center',padding:'28px 0' }}>
                      <div style={{ fontSize:11,color:C.muted,marginBottom:16,lineHeight:1.7 }}>
                        {L('AI analysis provides: reduction recommendations, offset strategy, CSRD compliance check, industry benchmarks.','L\'analyse IA fournit: recommandations de réduction, stratégie de compensation, vérification CSRD, benchmarks sectoriels.')}
                      </div>
                      <button onClick={runAI} disabled={aiLoading||!(currentAudit.grandTotal>0)}
                        ...
                        {aiLoading?'⟳ '+L('Analyzing...','Analyse...'):L('Run AI Analysis','Lancer l\'analyse IA')}
                      </button>
                    </div>
                  )}
                </div>
