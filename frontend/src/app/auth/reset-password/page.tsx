          ...
          {success ? (
            ...
            <>
              ...
              <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ fontSize: 10, color: '#4A6278', fontFamily: 'JetBrains Mono, monospace', display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>NOUVEAU MOT DE PASSE</label>
                  <div style={{ position: 'relative' }}>
                    <input type={show ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required autoFocus
                      placeholder="8 characters minimum"
                      style={{ width: '100%', background: '#0D1117', border: '1px solid #1E2D3D', borderRadius: 7, color: '#E8EFF6', padding: '11px 40px 11px 14px', fontSize: 14, boxSizing: 'border-box', outline: 'none' }}
                ...
                <div>
                  <label style={{ fontSize: 10, color: '#4A6278', fontFamily: 'JetBrains Mono, monospace', display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>CONFIRMER LE MOT DE PASSE</label>
                  <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} required
                    placeholder="Répétez le mot de passe"
                    style={{ width: '100%', background: '#0D1117', border: `1px solid ${confirm && confirm !== password ? 'rgba(248,113,113,0.4)' : confirm && confirm === password ? 'rgba(0,255,148,0.35)' : '#1E2D3D'}`, borderRadius: 7, color: '#E8EFF6', padding: '11px 14px', fontSize: 14, boxSizing: 'border-box', outline: 'none' }}/>
